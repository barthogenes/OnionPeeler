...
export class Ascii85Decoder {
  ...
  public decode(inputStr: string): string {
    ...
    for (const chunk of chunkedString) {
      ...
      // Encode each chunk.
      decodedStr += this.decodeChunk(chunk.padEnd(5, 'u'));
    }
  ...
  private decodeChunk(inputStr: string): string {
    ...
    return this.convert32BitIntegerToString(inputNum);
  }

  private convert32BitIntegerToString(num: number): string {
