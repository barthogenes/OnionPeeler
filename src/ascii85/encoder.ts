...
export class Ascii85Encoder {
  ...
  public encode(inputStr: string): string {
    ...
    for (const chunk of chunkedString) {
      ...
      // Encode each chunk.
      encodedStr += this.encodeChunk(chunk.padEnd(4, "\0"));
    }
  ...
  private encodeChunk(inputStr: string): string {
    let s = '';
    const base = 85;
    let inputNum = this.convertStringTo32BitInteger(inputStr);
  ...
  private convertStringTo32BitInteger(str: string): number {
